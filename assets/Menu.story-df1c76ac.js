import { aE as defineNuxtComponent, as as _export_sfc, ax as openBlock, ay as createElementBlock, aG as createBaseVNode, aF as renderSlot, aS as normalizeClass, aL as defineStore, aX as index, a$ as deepEqual, b0 as mapActions, au as resolveComponent, aA as Fragment, az as renderList, aH as createTextVNode, aI as toDisplayString, aB as createVNode, aJ as createCommentVNode, aK as FontAwesomeIcon, aV as ref, av as createBlock, aT as withModifiers, aM as mapState, b1 as lodashExports, aw as withCtx, b2 as resolveDynamicComponent, at as defineComponent, aC as normalizeProps, aD as guardReactiveProps, __tla as __tla_0 } from "./vendor-c1ea6bed.js";
import { D as DateRange, __tla as __tla_1 } from "./DateRange-af1e4fdc.js";
import { N as NumberRange, __tla as __tla_2 } from "./NumberRange-ebb913ba.js";
import { S as SelectFilter, __tla as __tla_3 } from "./Select-aa414f48.js";
import { f as filterValueFactory, c as filterValuesIsSet, i as isSet, d as isMatch } from "./types-filters-1e0221da.js";
import { I as ItemList, __tla as __tla_4 } from "./ItemList-d766addf.js";
import { T as TeritorioIcon, __tla as __tla_5 } from "./TeritorioIcon-e896869c.js";
import { M as MapStyleEnum } from "./types-f83cf40c.js";
import { __tla as __tla_6 } from "./Category-0f8a4bcf.js";
import { __tla as __tla_7 } from "./Item-3f7eb06b.js";
import { __tla as __tla_8 } from "./TeritorioIconBadge-a9ef080f.js";
import { __tla as __tla_9 } from "./Group-24fa2fb3.js";
import { __tla as __tla_10 } from "./Link-70d795a1.js";
let Menu_story;
let __tla = Promise.all([
  ...
  function getPoiById(apiEndpoint, apiProject, apiTheme, poiId, options = {}) {
    return fetch(`${apiEndpoint}/${apiProject}/${apiTheme}/poi/${poiId}.${options.format || defaultOptions.format}?` + new URLSearchParams(stringifyOptions(options))).then((data) => {
      if (data.ok) {
        return data.json();
      } else {
  ...
  function getPoiByCategoryIdUrl(apiEndpoint, apiProject, apiTheme, categoryId, options = {
    geometry_as: "point"
  }) {
    return `${apiEndpoint}/${apiProject}/${apiTheme}/pois/category/${categoryId}.${options.format || defaultOptions.format}?` + new URLSearchParams(stringifyOptions(options));
  }
  function getPoiByCategoryId(apiEndpoint, apiProject, apiTheme, categoryId, options = {
    geometry_as: "point"
  }) {
    return fetch(getPoiByCategoryIdUrl(apiEndpoint, apiProject, apiTheme, categoryId, options)).then((data) => {
      if (data.ok) {
        return data.json();
      } else {
        return Promise.reject(new Error([
  ...
  const menuStore = defineStore("menu", {
    ...
    actions: {
      ...
      async fetchFeatures({ apiEndpoint, apiProject, apiTheme, categoryIds }) {
        ...
        try {
          ...
          const posts = (await Promise.all(categoryIds.filter((categoryId) => !previousFeatures[categoryId]).map((categoryId) => getPoiByCategoryId(apiEndpoint, apiProject, apiTheme, categoryId, {
  ...
  const _sfc_main$3 = defineNuxtComponent({
    ...
    methods: {
      ...
      onPoiClick(searchResult) {
        getPoiById(this.$vidoConfig().API_ENDPOINT, this.$vidoConfig().API_PROJECT, this.$vidoConfig().API_THEME, searchResult.id).then((poi) => {
      ...
      search_() {
        if (this.searchText) {
          ...
          if (searchText.length === 2) {
            ...
            getPoiById(this.$vidoConfig().API_ENDPOINT, this.$vidoConfig().API_PROJECT, this.$vidoConfig().API_THEME, `cartocode:${cartocode}`).then((poi) => {
            ...
            const MenuItemsFetch = fetch(`${this.$vidoConfig().API_SEARCH}?${projectTheme}&type=menu_item&${query}`).then((data) => data.ok ? data.json() : null);
            const poisFetch = fetch(`${this.$vidoConfig().API_SEARCH}?${projectTheme}&type=poi&${query}&limit=10`).then((data) => data.ok ? data.json() : null);
            const addressesFetch = fetch(`${this.$vidoConfig().API_SEARCH_ADDR}?${query}`).then((data) => data.ok ? data.json() : null);
            Promise.all([
              MenuItemsFetch,
              poisFetch,
              addressesFetch
